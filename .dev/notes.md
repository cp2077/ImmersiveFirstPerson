...
- A trial freelook lock using `headingLocked` plus a narrow `pitchMin`/`pitchMax` window was rejected: REDengine forced a repeatable roughly `-0.05` degree native correction on entry. CET `Observe` is an `ObserveBefore` alias, so camera actions remain consumed before `PlayerPuppet.OnAction` without changing component parameters.
- The 2.31 camera-state audit is in `notes/camera-state-audit.md`. REDengine has several higher-priority camera owners inside Tier 1/2; body, freelook, and experimental height need separate compose/transfer/freeze/yield policies rather than a shared eligibility gate.
